  ...
  return (
    <section id="experience" className="relative w-full py-20 px-4 max-w-5xl mx-auto">
      ...
      <div className="mb-16 text-center">
        <div className="inline-flex items-center gap-2 font-mono text-xs text-red-500 tracking-widest uppercase mb-2">
          <Briefcase className="h-4 w-4" />
          <span>FLIGHT COMPUTER LOGS // STARK MISSION HISTORY</span>
        </div>
        <h2 className="font-orbitron text-3xl sm:text-5xl font-black text-white text-glow-crimson">
          MISSION HISTORY
        </h2>
        <div className="h-1 w-24 bg-gradient-to-r from-red-500 via-amber-400 to-cyan-400 mx-auto mt-4 rounded-full" />
      </div>

      {/* Flight Log Timeline Stream */}
      <div className="relative border-l-2 border-amber-500/30 ml-4 sm:ml-8 pl-6 sm:pl-10 space-y-8">
        {EXPERIENCES.map((exp) => {
            ...
            <div key={exp.id} className="relative group">
              ...
              <div
                className={`absolute -left-[31px] sm:-left-[47px] top-1.5 h-6 w-6 rounded-full border-2 transition-all duration-300 flex items-center justify-center ${
                  exp.status === 'ACTIVE'
                    ? 'border-amber-400 bg-amber-500/20 shadow-[0_0_15px_#ffd700]'
              ...
              <div
                ...
                className="interactive glass-panel glass-panel-hover corner-brackets rounded-xl p-6 cursor-pointer border-amber-500/30"
              >
                {/* Log Header */}
                ...
                <p className="font-mono text-sm text-cyan-400 mb-4">{exp.company}</p>
                ...
                {isExpanded && (
                  <div className="pt-4 border-t border-amber-500/20 space-y-4">
                    <div>
                      <div className="font-mono text-xs font-bold text-amber-300 mb-2">
                        // KEY STARK MISSION ACHIEVEMENTS:
                      </div>
                    ...
                    <div>
                      <div className="font-mono text-xs font-bold text-slate-400 mb-2">
                        // DEPLOYED AVIONICS STACK:
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {exp.techStack.map((tech) => (
                          <span
                            key={tech}
                            className="font-mono text-xs rounded bg-slate-900 px-2.5 py-1 text-amber-300 border border-amber-500/30"
                          >
                            {tech}
                          </span>
                        ))}
                ...
                {/* Toggle Footer */}
                <div className="mt-4 flex items-center justify-between font-mono text-xs text-amber-400/90 pt-2 border-t border-slate-800">
